...
parser.add_argument('--num_tasks', type=int, default=6, help='number of tasks for continual learning')
parser.add_argument('--batch_size', type=int, default=256, help='number of data points in a batch')
parser.add_argument('--lr', type=float, default=1e-3, help='learning rate')
parser.add_argument('--num_epochs', type=int, default=80, help='number of training epochs')
parser.add_argument('--num_points', type=int, default=200, help='number of inducing points for each task')
...
def main(args):
    ...
    acc = train_cifar(num_tasks=args.num_tasks, batch_size=args.batch_size, lr=args.lr, num_epochs=args.num_epochs,
                      num_points=args.num_points, use_cuda=use_cuda, select_method=args.select_method, tau=args.tau)
    ...
    for i in range(args.num_runs):
        ...
        if save_results:
            ...
            torch.save({
                'args_list': args_list,
                'accs_list': acc_list,
            }, save_path + 'cifar_seed_%d.tar' % (args.seed))
