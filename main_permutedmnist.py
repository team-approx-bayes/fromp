from train_permutedmnist import train_permutedmnist
import argparse
import torch
import numpy as np
import copy
...
parser.add_argument('--hidden_size', type=int, default=100, help='network hidden layer size (2 hidden layers)')
...
parser.add_argument('--num_points', type=int, default=200, help='number of memorable points for each task')
...
def main(args):
    ...
    acc = train_permutedmnist(num_tasks=args.num_tasks, batch_size=args.batch_size, hidden_size=args.hidden_size,
                              lr=args.lr, num_epochs=args.num_epochs, num_points=args.num_points,
                              use_cuda=use_cuda, select_method=args.select_method, tau=args.tau)
...
if __name__ == '__main__':
    ...
    for i in range(args.num_runs):
        ...
        print('\nPermuted MNIST, seed', args.seed+i)
        ...
        if save_results:
            ...
            torch.save({
                'args_list': args_list,
                'accs_list': acc_list,
            }, save_path + 'permuted_seed_%d.tar' % (args.seed))
